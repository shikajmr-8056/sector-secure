...
function ExpandedRow({
  ...
  f: Finding;
  sector: Sector;
  onFixApplied: (postFixScore: number) => void;
}) {
  ...
  const handleApplyFix = () => {
    if (!fixResult) return;
    // Bubble up to scan.tsx — this mutates findings state there, which
    // cascades to stats bar, heatmap, treemap, before/after panel
    onFixApplied(fixResult.postFixScore);
    // Collapse the fix panel — the row in the table now shows the fixed state
    setShowFix(false);
  };

  return (
    <div className="border-t border-border bg-background/40 px-4 py-4 space-y-4">
      {/* ── Code snippet + meta grid ── */}
        ...
        <div className="space-y-3">
          ...
          {isFixed && (
            <div className="flex items-center gap-2 rounded-md border px-3 py-2"
              style={{ borderColor: "color-mix(in oklab, var(--sev-low) 40%, transparent)", background: "color-mix(in oklab, var(--sev-low) 8%, transparent)" }}>
              <span style={{ color: "var(--sev-low)" }}>✓</span>
              <span className="font-mono text-[11px]" style={{ color: "var(--sev-low)" }}>Fix applied — dashboard updated</span>
            </div>
          )}
        </div>
      </div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            {loadingFix ? (
              ...
              <div className="space-y-3">
                ...
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}
                  ...
                  <div className="border-b border-border bg-panel/60 px-4 py-2.5 flex items-center justify-between">
                    ...
                    <button onClick={handleApplyFix}
                      className="rounded-md bg-primary px-3 py-1 font-mono text-[10px] font-bold text-background hover:bg-primary/90 transition-colors">
                      Apply fix → update dashboard
                    </button>
                  </div>
                ...
                {/* Score preview — shown BEFORE apply, disappears after */}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.22 }}
                  className="flex items-center gap-4 rounded-xl border px-5 py-3"
                  style={{
                    borderColor: "color-mix(in oklab, var(--sev-low) 25%, transparent)",
                    background: "color-mix(in oklab, var(--sev-low) 5%, transparent)",
                  }}>
                  <div className="flex items-baseline gap-1.5 font-mono">
                    <span className="text-lg font-black" style={{ color: BUCKET_COLOR[bucket] }}>{liveScore.toFixed(1)}</span>
                    <span className="text-muted-foreground">→</span>
                    <span className="text-lg font-black" style={{ color: "var(--sev-low)" }}>{fixResult.postFixScore.toFixed(1)}</span>
                  </div>
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-sev-low" style={{ width: `${(fixResult.postFixScore / 10) * 100}%` }} />
                  </div>
                  <span className="font-mono text-[10px] font-bold" style={{ color: "var(--sev-low)" }}>
                    −{fixResult.scoreReduction.toFixed(1)} if applied
                  </span>
                </motion.div>
...
export function FindingsTable({
  ...
  findings: Finding[];
  onFixApplied: (findingId: string, postFixScore: number) => void;
}) {
  ...
  // Count how many fixes have been applied
  const fixedCount = findings.filter(f => f.fixApplied).length;
          ...
          return (
            <motion.div key={f.id} layout
              ...
              <button onClick={() => setOpen(isOpen ? null : f.id)}
                ...
                {/* Severity bar — green when fixed */}
                <div className="h-8 w-0.5 shrink-0 rounded-full"
                  style={{ background: isFixed ? "var(--sev-low)" : BUCKET_COLOR[bucket] }} />

                <div className="min-w-0 flex-1">
                  <p className={`truncate text-sm ${isFixed ? "line-through text-muted-foreground" : "text-foreground"}`}>{f.title}</p>
                  <p className="mt-0.5 truncate font-mono text-[11px] text-muted-foreground">
                    {f.filePath}:{f.lineNumber} ·{" "}
                    <span style={{ color: SOURCE_COLOR[f.source ?? ""] ?? "var(--muted-foreground)" }}>{f.source ?? f.tool ?? "unknown"}</span>
                    {f.cveId && <span className="ml-2 text-muted-foreground/60">{f.cveId}</span>}
                    {isFixed && <span className="ml-2 font-medium" style={{ color: "var(--sev-low)" }}>✓ fixed</span>}
                  </p>
                </div>
                ...
                {isFixed && (
                  <span className="hidden shrink-0 rounded-full border px-2 py-0.5 font-mono text-[10px] sm:block"
                    style={{ color: "var(--sev-low)", borderColor: "var(--sev-low)" }}>fixed</span>
                )}

                <div className="flex shrink-0 items-baseline gap-2 font-mono tabular-nums">
                  {/* Always show original base as struck */}
                  <span className="text-xs text-muted-foreground line-through">{base.toFixed(1)}</span>
                  {isFixed ? (
                    /* After fix: show post-fix score in green, old AVSS struck */
                    <div className="flex items-baseline gap-1">
                      <span className="text-xs line-through" style={{ color: scoreColor(live) }}>{live.toFixed(1)}</span>
                      <motion.span
                        initial={{ opacity: 0, scale: 0.7 }}
                        animate={{ opacity: 1, scale: 1 }}
                        className="text-base font-bold"
                        style={{ color: "var(--sev-low)" }}>
                        {(f.postFixScore ?? 0).toFixed(1)}
                      </motion.span>
                    </div>
                  ) : (
                ...
                {isOpen && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    ...
                    <ExpandedRow
                      f={f}
                      sector={sector}
                      onFixApplied={(postFixScore) => onFixApplied(f.id, postFixScore)}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
